...
document.querySelector("#enterUser").addEventListener("click", addEmployee);
...
function addEmployee(event) {
  ...
  if (name.trim() === "" || profession.trim() === "" || age.trim() === "") {
    document.getElementById("error").innerHTML =
      "Error : Please Make sure All the fields are filled before in an employee !";
...
function displayEmployee() {
  const addedEmployeesContainer = document.getElementById("employeeAdded");
  addedEmployeesContainer.innerHTML = " ";

  employees.forEach((employee) => {
    const employeeOuterContainer = document.createElement("div");
    employeeOuterContainer.className = "employeeDiv";
